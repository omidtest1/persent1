<?php
// ------------------------------
// ماتریس اعطا/سلب مجوز کاربران و گروه‌ها (درختی و آبشاری + چک‌باکس)
// نمایش وضعیت دسترسی بالای صفحه
...
// -- Helpers for fetch tree and lists --
    ...
    if ($all_perm_ids) {
        ...
        foreach($selected_groups as $gid) {
            ...
            foreach($all_perm_ids as $pid) {
                $p = $permissions_by_id[$pid];
                $lack = [];
                foreach($members as $uid) if (!user_has_permission($uid, $p['name'])) $lack[] = $users_by_id[$uid]['username'];
                if (!$lack)
