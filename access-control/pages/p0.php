...
// کاملاً ریسپانسیو و زیبا (Bootstrap 5.3)
// ------------------------------
...
if ($_SERVER['REQUEST_METHOD']==='POST' && $action && (count($selected_users) || count($selected_groups)) && (count($selected_perms) || count($selected_perm_cats))) {
    // لیست همه مجوزها
...
if (count($selected_users) || count($selected_groups)) {
    ...
    if ($all_perm_ids) {
        ...
        foreach ($selected_users as $uid) {
            ...
            if (!$lacks)
                $status_lines[] = "<span class='text-success'>کاربر <b>".htmlspecialchars($user['username'])."</b> به همه مجوزهای انتخابی دسترسی دارد.</span>";
            else
                $status_lines[] = "<span class='text-danger'>کاربر <b>".htmlspecialchars($user['username'])."</b> به این مجوزها دسترسی ندارد: <b>".implode(', ',$lacks)."</b></span>";
        }
        // تک تک گروه‌ها
        foreach($selected_groups as $gid) {
            $g = $groups_by_id[$gid];
            ...
            if (!$members) {
                $status_lines[] = "<span class='text-warning'>گروه <b>".htmlspecialchars($g['name'])."</b> بدون عضو است.</span>";
                continue;
            }
            foreach($all_perm_ids as $pid) {
                $p = $permissions_by_id[$pid];
                ...
                if (!$lack)
                    $status_lines[] = "<span class='text-success'>کل اعضای گروه <b>".htmlspecialchars($g['name'])."</b> به مجوز <b>".htmlspecialchars($p['label']?:$p['name'])."</b> دسترسی دارند.</span>";
                else
                    $status_lines[] = "<span class='text-danger'>در گروه <b>".htmlspecialchars($g['name'])."</b> این کاربران به مجوز <b>".htmlspecialchars($p['label']?:$p['name'])."</b> دسترسی ندارند: <b>".implode(', ',array_map('htmlspecialchars',$lack))."</b></span>";
            }
        }
        $status_msg = implode("<br>", $status_lines);
    }
}
...
<head>
    ...
    <style>
        ...
        .panel-actions { min-height: 130px; display: flex; flex-direction: column; align-items: stretch; justify-content: center;}
...
<div class="container panel-main">
    ...
    <?php if($status_msg): ?><div class="alert alert-<?= $success?'success':'danger' ?>"><?= $status_msg ?></div><?php endif; ?>
    <form method="post" class="row g-2">
        <!-- ستون راست: کاربران و گروه‌ها -->
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="side-title mb-0">کاربران و گروه‌ها</div>
            <div class="p-3 pb-2 bg-light rounded-bottom shadow-sm tree" style="min-height:200px;max-height:440px;overflow:auto;">
        ...
        <div class="col-lg-2 col-md-12 d-flex flex-column align-items-center panel-actions mb-4">
            <button name="action" value="grant" class="btn btn-success mb-3 py-2" style="font-size:1.09em">اعطای این مجوزها</button>
            <button name="action" value="revoke" class="btn btn-danger py-2" style="font-size:1.09em">سلب این مجوزها</button>
        </div>
...
<!-- اسکریپت انتخاب گروه/کاربر والد -->
<script>
