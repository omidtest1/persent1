...
if($action === "vote_paper") {
    ...
    if($attendance_status=='out') {
        $err_msg = "این عضو قبلاً خروج ثبت کرده است و اکنون در جلسه نیست. آیا مطمئن هستید می‌خواهید برگه رأی صادر کنید؟";
    } elseif($attendance_status=='none') {
        $err_msg = "برای این عضو هیچ ورود یا خروجی ثبت نشده! آیا مطمئن هستید می‌خواهید برگه رأی صادر کنید؟";
    }
    if(($err_msg && $config['modal_confirm_vote_paper']) && !$force) {
        echo json_encode([
            'confirm_modal'=>true,
    ...
    if($err_msg && $config['admin_only_vote_paper_abnormal'] && !$is_admin) {
    ...
    if($mode === 'system') {
        // تولید سریال/بارکد یکتا (شماره سریال)
        ...
        $body = "<div class='mb-3'><b>برگه رأی صادر شد. پس از چاپ و تحویل، بارکد را با بارکدخوان اسکن و تایید کنید.</b></div>
        <div id='votePaperDisplay'>$vote_paper_html</div>
        <input type='text' class='form-control' id='barcodeInput' autofocus autocomplete='off' style='font-size:1.3em;direction:ltr;text-align:center;max-width:220px;margin:10px auto 0;' placeholder='بارکد را اسکن کنید'>
        <div class='mt-3' id='barcodeError' style='color:#d32f2f;'></div>
        ";
        $footer = "<button class='btn btn-secondary btn-cancel' data-bs-dismiss='modal'>انصراف</button>
                   <button class='btn btn-success modal-assign-barcode-btn' data-id='$member_id' data-barcode='$barcode'>ثبت برگه رأی</button>";
        echo json_encode([
            'confirm_modal'=>true,
            'modal_title'=>"صدور و ثبت برگه رأی",
            'modal_body'=>$body,
            'modal_footer'=>$footer,
            'vote_mode'=>'system',
            'barcode'=>$barcode
...
// ثبت اسکن بارکد برگه رأی برای عضو
if($action === "assign_vote_paper") {
    ...
    echo json_encode(['message'=>'برگه رأی با موفقیت ثبت شد','refresh'=>true]);
    exit;
}
...
// ---------------- جزییات کامل با دکمه عکس و فوکوس‌پذیر ----------------
if($action === "details") {
